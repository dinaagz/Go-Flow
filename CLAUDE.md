...
- `DP` — 34 default products across 8 categories (`grp` key groups identical products from different suppliers, e.g. `pdt-grand`, `pdt-dome`). Products from the 7.6/7.7 quotes carry an `imgs` gallery array (up to 3 photos, shown as clickable thumbnails on the product card via `cardGallery`/`swapCardImg`). **`DP` is only the seed catalogue for a brand-new browser** — since there's no shared backend, each user's real catalogue lives in their own `gf_p` (localStorage/IndexedDB) and grows independently as they add products (e.g. via bulk import from Paine Agent Sourcing); it is *not* kept in sync with `DP` and can diverge significantly. Don't assume `DP`'s product count reflects any given user's actual catalogue — check a fresh backup export (Paramètres → Exporter mes données) if you need the real numbers
