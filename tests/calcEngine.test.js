...
// Le moteur vit dans index.html (app single-file, pas de build) : ce fichier
// extrait le bloc source entre les marqueurs MOTEUR DE CALCUL / FIN MOTEUR et
// l'exécute dans un contexte VM isolé, pour tester la logique réelle sans dupliquer le code.
//
...
function loadCalcEngine() {
  ...
  if (start === -1 || end === -1) {
    throw new Error('Marqueurs MOTEUR DE CALCUL introuvables dans index.html — a-t-il été renommé/déplacé ?');
  }
